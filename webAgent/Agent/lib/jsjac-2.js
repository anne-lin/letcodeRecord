...
reqstr = '<open xmlns="' + NS_FRAMING + '" to="' + streamto + '" version="1.0"/>';return reqstr;};JSJaCWebSocketConnection.prototype.send=function(packet,cb,arg){if (!this.connected()) {return false;}if(!packet || !packet.pType){this.oDbg.log('no packet: ' + packet, 1);return false;}this._ws.onmessage = JSJaC.bind(this._onmessage, this);if(cb){if(!packet.getID()){packet.setID('JSJaCID_'+this._ID++);}
...
reqstr = '<open xmlns="' + NS_FRAMING + '" to="' + streamto + '" version="1.0"/>';this._sendRaw(reqstr,cb);};JSJaCWebSocketConnection.prototype._sendRaw=function(xml,cb,arg){if(!this._ws){return false;}if(cb){this._ws.onmessage=JSJaC.bind(cb,this,arg);}
