...
define(["TLog","VertoFun","MultiRegisterEvent"], function(Log,VertoFun,MultiRegisterEvent) {
    ...
    var vertoCallback = {
        ...
        onWSClose: function (data,isLogin) {
            // isLogin:主动退出返回的是false
            // if(WebAgent.isMulticenter && errorLoginNum == WebAgent.webRTCConnectNum){
            //     // 重连
            //     WebAgent.currentWebRTCIndex ++ ;
            //     if(WebAgent.currentWebRTCIndex >= WebAgent.config['webRTCUrlList'].length) WebAgent.currentWebRTCIndex = 0;
            //     console.log('WebRTC:第五次重连，将切换WebRTC的地址');
            //     WebAgent.config["vertoWebSocket"] = WebAgent.config['webRTCUrlList'][WebAgent.currentWebRTCIndex];
            //     WebAgent.config['iceServers'] = WebAgent.config["iceServersList"][WebAgent.currentWebRTCIndex];
            //     data.rpcClient.options.socketUrl = WebAgent.config["vertoWebSocket"];
            //     data.rpcClient.options.iceServers = WebAgent.config['iceServers'];
            //     errorLoginNum = 0;
            // }
            // errorLoginNum ++;
            // if(!isLogin){
    ...
    function login(username, password, socketUrl, callback , logoutCallbackRegister) {
        ...
        RTC.login(Object.assign({
            ...
            //     credential: "test001",
            // }]
            iceServers:WebAgent.config['iceServers']
