...
(function(window) {
    ...
    function configInit(){
        let url = WebAgent.host.replace(/^(https?:\/\/)/,'');
        var config = {
            weixinEmojiPath: "https://" + url +"/emotion/",//表情文件地址
            quickSmartReplyPath_sensitive: "https://"+ url + "/dcmse",//快捷&智能回复&敏感词校验接口地址
            getProjectDataPath: "https://"+ url + "/customWebservice/mvc",//在线客服数&排队客户数&消息记录&当天下线客户列表
            // HTTP_BASE: "http://open.ccod.com:7070/http-bind/",//openfire_base
            ADD_AGENT_URL: "https://"+ 'rtctest.channelsoft.com' + "/plugins/registerPlugin",//添加用户url
            FILE_UPLOAD_URL: "https://"+ url + "/upload/WeixinFileUpload",// 上传文件接口地址
            DOWN_FILE_URL: "https://"+ url + "/"//下载文件IP
        };
        WebAgent.config = $.extend(WebAgent.config || {},config);
        WebAgent.config["HTTP_BASE"]="wss://"+ 'rtctest.channelsoft.com' + "/ws/";
        return true;
    }
    WebAgent.configInit = configInit;
    WebAgent.configInit();

    WebAgent.config["HTTP_SERVER"]="ccod";
    // WebAgent.config["HTTP_BASE"]="wss://rtctest.channelsoft.com/ws/";
    // WebAgent.config["vertoWebSocket"]="wss://rtctest.channelsoft1.com/rtcsock";
