...
define('wa_ViewModel',["jquery", "knockout", "require", "wa_Defaults", "wa_Constant", "wa_Log", "wa_Tip", "wa_RegisterHandler", "colorBlink", "wa_Counter", "dialog"], function ($, ko, require, defaults, constant, Log, tip, registerHandler, undefined, Counter, dialog) {
    ...
    return function () {
        ...
        this.awayEnabled = ko.pureComputed(function () {
            return !this.doingState() && (this.currentState() == constant.states.BUSY || this.currentState() == constant.states.READY || this.currentState() == constant.states.ACW || this.currentState() == constant.states.AWAY);
        }, this);
...
define('wa_Extend',["wa_Log", "wa_Constant", "wa_Vm", "wa_Socket", "wa_HandleEvent"], function(Log, constant, vm, socket, handleEvent) {
    ...
    extend.makeCall = function(params) {
        if (!vm.callEnabled()) { return { code : -1, msg : "make call denied" }; }

        params = params || {};
        if (!params.outCallNumber) { return { code : -2, msg : "呼叫号码必填" }; }

        if(params.isIntellectCall){
            params.disNumber = ""
        }else{
            if(!params.disNumber){
                return {code: -2, msg: '没有可用的外显号码'}
            }
        }

        vm.numberProtection(params && params.numberProtection);  //设置外呼号码是否被保护

        var outCallNumber;

        if(vm.numberProtection()) {
            vm.callNumber(numberProtection(params.outCallNumber));
        } else {
            vm.callNumber(params.outCallNumber);
        }

        outCallNumber = params.outCallNumber;
        //vm.disNumber(params.disNumber);     // 当params.disNumber存在于outNumbers集合中时才会赋值成功
    ...
    extend.consult = function(params) {
        ...
        socket.sendCommand({
            ...
            ext : {
                ...
                transrmode : params.type         // 1:内线咨询, 0:外线咨询
            }
        });
        return { code : 0 };
    };
    ...
    extend.singleConference = function(params) {
        ...
        socket.sendCommand({
            ...
            ext : {
                to : params.transferTo,
                transrmode : params.type    //1:会议呼叫内线，0:会议呼叫外线，2:会议呼叫技能组，4:工作态坐席
            }
        });
